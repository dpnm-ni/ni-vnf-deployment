...
optlist <- list(
    ...
    make_option(
        "--tgOutfile", type = "character", default = "requests-tg.txt",
        help = "Name of txt output file containing the SFC request trace with IPs rather than node IDs. [default is %default]."),
    make_option(
        "--topoFile", type = "character", default = "inet2",
        help = "Name of the topology file to use [default is %default]."),
    make_option(
        "--sfccatFile", type = "character", default = "sfccat.csv",
        help = "Name of the SFC catalog file to use [default is %default]."),
    make_option(
        "--sfcnodesFile", type = "character", default = "sfcnodes.csv",
        help = "Name of the SFC nodepool file to use [default is %default]."),
    make_option(
        "--ipmapFile", type = "character", default = "ipmap.csv",
        help = "Name of the IP map file to use [default is %default]."),
    make_option(
    ...
    make_option(
        "--minBandwidth", type = "integer", default = "5000",
        help = "Requests will have a bandwidth from the interval [minBandwidth; maxBandwidth]. Unit: kbps. [default is %default]."),
    make_option(
        "--maxBandwidth", type = "integer", default = "13000",
        help = "Requests will have a bandwidth from the interval [minBandwidth; maxBandwidth]. Unit: kbps. [default is %default]."),
    make_option(
    ...
    make_option(
        "--compressTime", type = "double", default = "1",
        help = "Compress time by a factor - use this to get a week's pattern over a shorter period. [default is %default]."),
    make_option(
        "--seed", type = "integer", default = "40",
        help = "RNG seed for reproducibility. [default is %default]."),
    make_option(
        "--dbg", type = "integer", default = "0",
        help = "Debug mode. If dbg == i, then only the first i requests are returned rather than the whole trace. [default is %default]."))
...
sfccat <- fread(opt$sfccatFile) %>%
    mutate(sfc = strsplit(sfc, "\\|"))
...
for (i in 1:nrow(sfccat)) {
    ...
    curArrivaltimes <- switch (iatDist,
                               ...
                               "norm" = cumsum(rnorm(n = curNRequests, mean = 1 / curMinrate, sd = 0)))
...
duration <- switch (durDist,
    "negexp" = round(200 + rexp(nrow(requests), 1 / (opt$meanDuration - 100))),
    "norm" = round(opt$meanDuration + 0*rnorm(nrow(requests), mean = 0, sd = opt$meanDuration / 4)))

nneg <- sum(duration < 0)
if (nneg > 0) {
    warning(sprintf("Found %d negative durations, applying abs() - might need to check input parameters!\n", nneg))
    duration <- abs(duration)
}

requests$duration <- round(duration / opt$compressTime)
requests$src <- 0
requests$dst <- 0
...
requests$traffic <- round(opt$minBandwidth + runif(n = nrow(requests)) * (opt$maxBandwidth - opt$minBandwidth))
requests$maxlat <- round(700 + runif(n = nrow(requests)) * 50)

# List of node ID pools for each SFC.
sfcid2nodepools <- fread(opt$sfcnodesFile) %>%
    mutate(
        srcnodes = strsplit(srcnodes, "\\|"),
        dstnodes = strsplit(dstnodes, "\\|"))

for (i in 1:nrow(requests)) {
    # Get SFC ID of current request to identify allowed set of nodes.
    # curSFCID <- requests$sfcid[i]
    # 
    # 
    # nodepool <- sfcid2nodepools %>% filter(sfcid == curSFCID)
    # fromto <- c(
    #     sample(nodepool$srcnodes %>% unlist(), size = 1),
    #     sample(nodepool$dstnodes %>% unlist(), size = 1)) %>%
    #     as.numeric()
    # 
    # requests$src[i] <- fromto[1]
    # requests$dst[i] <- fromto[2]

    allowednodes <- switch (topofile,
                            # All nodes are allowed in the case of Internet2.
                            "inet2" = c(1,2,3,4,5,6,7,8,9),#1:nnodes,
...
if (length(which(requests$src == requests$dst)) > 0) {
    warning(sprintf("Found requests with src == dst, proceed with caution (not compatible with ILP)."))
}
...
requests <-
    ...
    mutate(arrivaltime = arrivaltime / opt$compressTime)

# FIXME: cutoff trace to check pipeline.
dbg <- opt$dbg
if (dbg > 0) {
    warning(sprintf("Only outputting first %d requests.", dbg))
    requests <- 
        requests %>% 
        head(dbg)
}
...
write.table(
    requests %>%
        select(-one_of("sfcid")) %>% 
        # Round / reformat arrival times for compatibility with CPLEX.
        mutate(arrivaltime = round(arrivaltime)) %>% 
        mutate(arrivaltime = sprintf("%d", arrivaltime), traffic = sprintf("%d", traffic)),
    file = opt$txtOutfile, sep = ",", col.names = F, row.names = F, quote = F)

# For trafgen (with IPs rather than node IDs).
id2ip <- fread(opt$ipmapFile)

requests.ip <-
    requests %>%
    inner_join(id2ip, by = c("src" = "id")) %>%
    mutate(src = ip) %>%
    select(-one_of("ip")) %>%
    inner_join(id2ip, by = c("dst" = "id")) %>%
    mutate(dst = ip) %>%
    select(-one_of("ip")) %>% 
    # Trafgen uses integers that represent Mbps.
    mutate(traffic = round(as.numeric(traffic) / 1000))

write.table(requests.ip %>% select(-one_of("sfcid")), file = opt$tgOutfile, sep = ",", col.names = F, row.names = F, quote = F)
